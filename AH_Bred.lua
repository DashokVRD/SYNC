	...
	if i_re_menu.v and control_recon and recon_to_player and setting_items.hide_td.v then
		...
		local btn_size = imgui.ImVec2(-0.1, 0)
			...
			if accept_load then
				...
				if imgui.ToggleButton(u8"Игроки рядом", setting_items.player_nearby) then
				if player_nearby_fnc then
				player_nearby_fnc = false
				else
				player_nearby_fnc = true
				end
				else
			...
			traic = true
		       sampAddChatMessage(tag .. "Трейсера включены.")
				...
				for _, v in pairs(playerid_to_stream) do
					if imgui.Button(" - " .. sampGetPlayerNickname(v) .. "[" .. v .. "] - ") then
						sampSendChat("/re " .. v)
					end				
				end
				end
