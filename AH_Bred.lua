...
local script_version_text = "8.00"
...
function imgui.OnDrawFrame()
	...
	if i_ans_window.v then
	   ...
	   lua_thread.create(function()
	   sampSendDialogResponse(2351, 1, 0,"{FFFFFF}Передам ваш репорт администрации")
	   wait(700)
	   sampCloseCurrentDialogWithButton(1)
	   wait(700)
...
function explode_argb(argb)
	...
	if i_setting_items.v then
		...
		local btn_size = imgui.ImVec2(-0.1, 0)
		imgui.Begin(u8" Основные настройки скрипта.", i_setting_items)
	...
	reloadScripts()
		...
		imgui.Text(u8"Данный скрипт перезагружает все скрипты в папке moonloader")
		imgui.Text(u8"Активация: CTRL + R")
		...
		lua_thread.create(function()
		sampAddChatMessage(tag .. string.format("Установленно %d из %d", p1, p2))
        wait(900)
		wait(900)
		reloadScripts()
    end)
	...
	imgui.TextColoredRGB(doesFileExist(getGameDirectory().."//moonloader//reload_all.lua") and u8"{62E511}Установлен" or u8"{F70303}Не установлен")
	imgui.SameLine()
...
imgui.ShowCursor = false
		...
		imgui.Begin(u8"Статистика", stats_window_imgui, imgui.WindowFlags.NoResize)
		imgui.TextColoredRGB(u8"Репортов за сегодня: " ..defTable.setting.report_day)
		imgui.TextColoredRGB(u8"Репортов всего: " .. defTable.setting.report_all)
		imgui.TextColoredRGB(u8"Время в игре: ".. get_clock(time))
		...
		local btn_size = imgui.ImVec2(-0.1, 0)
			...
			traic = true
		       sampAddChatMessage(tag .. "Трейсера включены.")
				...
				if imgui.Button(u8"Проверить клиент") then
				lua_thread.create(function()
	...
	local sw2, sh2 = getScreenResolution()
		imgui.LockPlayer = true
		imgui.SetNextWindowPos(imgui.ImVec2(sw2 / 2.5, sh2 / 2.5), imgui.Cond.FirstUseEver, imgui.ImVec2(0, 0))
...
if not isSampfuncsLoaded() or not isSampLoaded() then return end
	...
	report_answer = "[A]" .. player_nick .. "[" .. player_id .. "] ответил"
...
local delay = 500
lua_thread.create(function()
sampAddChatMessage(tag .. "Запускаю формы из файла forms.txt")
wait(1000)
if fileForms then
...
if fileForms == nil then 
	  ...
	  sampAddChatMessage(tag .. "Отсутствует файл forms.txt, создаю файл.")
      -- Закрывает файл
      fileForms:close();
   end;
...
function goupdate()
sampAddChatMessage(tag ..'Обнаружено новое обновление')
